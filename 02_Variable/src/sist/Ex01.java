 ...
 * - 필요한 데이터를 저장할 곳을 미리 선언해 주고, 나중에 계속해서 사용하기위해 변수를 만든다.
 * 
 * 변수 선언 방법
 * - 형식) 자료형 변수명;
 *      - 자료형 : 기본 자료형(8가지)
...
/*
 * 자바에서의 기본 자료형(8가지)
 * 1. 정수형 : byte(1바이트), short(1바이트), int(4바이트), long(8바이트)
 ...
 * 1. 예약어(키워드 - 문법적으로 사용하지 않는 단어) 사용불가
 * 2. 영문자 또는 특수문자(_ or &)로 시작.
 * 3. 숫자로 시작하면 안됨 - 1sum(X)
 * 4. 대소문자 구별함 - SUM고 sum은 다른 변수명으로 인식함.
