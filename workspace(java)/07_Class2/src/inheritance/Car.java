 ...
 * - 실세계에서 상속은 상위 개체의 속성이 하위 개체에 물려져서 하위 개체가 상위 개체의 속성을 모두 가지는 관계임.
 * - 자바의 상속은 자식 클래스가 부모 클래스의 속성을 물려 받고 기능을 추가하여 확장(extends)하는 개념임.
 ...
 *   ==> 자식은 부모의 멤버보다 같거나 많다.
 * - 상속의 대상을 멤버(멤버변수, 멤버메서드)
 *   ==> 단, 생성자나  private 접근 제한을 갖는 멤버변수와 멤버메서드는 상속에서 제외됨.
 * - 상속 시 사용되는 키워드 : extends
 * - 상속은 단일 상속만 가능함.
 *   ==> 상속을 받을 수 있는 클래스는 하나이다.
 * - 상속은 클래스의 수정을 최소화 시킬 수 있고, 유지보수의 시간을 최소화시켜 준다는 장점이 있음.
 * 
 *   형식) [접근제한] class 자식 클래스명 extends 부모클래스명 {...}
 * - 상속관계 : '~은 ~이다.(is a 관계)'
 *   포함관계 : '~은 ~를 가지고 있다.(has a 관계)'
 *           ==> 사람은(name, age, id) 가지고 있다.
 * - 주의사항) 부모 클래스는 반드시 기존에 미리 정의되어 있어야 함.
 */
